...
from data import (TokenBucketSampler, TokenBucketSamplerForItm,
                  ...
                  TxtTokLmdb, ImageLmdbGroup, ConcatDatasetWithLens, DetectFeatLmdb,
                  MlmDataset, MrfrDataset, MrcDataset,
                  mlm_collate, mrfr_collate, mrc_collate,
                  ItmDataset, itm_collate, itm_ot_collate, VcrTxtTokLmdb, itkm_otk_collate,
                  ItkmDataset, MkmDataset, mkm_collate, itkm_collate)
...
# ok!
def build_mlm_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    dataset_list = []
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        ...
        dataset_list.append(MlmDataset(qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa", isTrain=isTrain))
        dataset_list.append(MlmDataset(qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
    collate_fn = mlm_collate
    dataset = ConcatDatasetWithLens(dataset_list)
    return dataset, collate_fn

# ok!
def build_mrfr_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    dataset_list = []
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        img_db, img_db_gt = load_img_feat(img_path, all_img_dbs, opts)
        qa_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qa")
        qar_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qar")
        dataset_list.append(MrfrDataset(opts.mrm_prob, qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa", isTrain=isTrain))
        dataset_list.append(MrfrDataset(opts.mrm_prob, qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
    collate_fn = mrfr_collate
    dataset = ConcatDatasetWithLens(dataset_list)
    return dataset, collate_fn

# ok!
def build_mrc_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    dataset_list = []
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        img_db, img_db_gt = load_img_feat(img_path, all_img_dbs, opts)
        qa_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qa")
        qar_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qar")
        dataset_list.append(MrcDataset(opts.mrm_prob, qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa" , isTrain=isTrain))
        dataset_list.append(MrcDataset(opts.mrm_prob, qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
...
def build_itm_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    ...
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        ...
        qa_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qa")
        qar_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qar")
        dataset_list.append(ItmDataset(qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa", isTrain=isTrain))
        dataset_list.append(ItmDataset(qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
    collate_fn = itm_ot_collate
    dataset = ConcatDatasetWithLens(dataset_list)
    return dataset, collate_fn

# ok!
def build_itkm_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    dataset_list = []
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        img_db, img_db_gt = load_img_feat(img_path, all_img_dbs, opts)
        qa_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qa")
        qar_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qar")
        dataset_list.append(ItkmDataset(qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa", isTrain=isTrain))
        dataset_list.append(ItkmDataset(qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
    #collate_fn = itkm_otk_collate
    collate_fn = itkm_collate
    dataset = ConcatDatasetWithLens(dataset_list)
    return dataset, collate_fn

# ok!
def build_mkm_dataset(txt_dbs, img_dbs, all_img_dbs, isTrain, opts):
    dataset_list = []
    for txt_path, img_path in zip(txt_dbs, img_dbs):
        img_db, img_db_gt = load_img_feat(img_path, all_img_dbs, opts)
        qa_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qa")
        qar_txt_db = VcrTxtTokLmdb(txt_path, opts.max_txt_len, task="qar")
        dataset_list.append(MkmDataset(opts.mkm_prob, qa_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qa", isTrain=isTrain))
        dataset_list.append(MkmDataset(opts.mkm_prob, qar_txt_db, img_db_gt=img_db_gt, img_db=img_db, task="qar", isTrain=isTrain))
    collate_fn = mkm_collate
    dataset = ConcatDatasetWithLens(dataset_list)
    return dataset, collate_fn
    ...
    for dset in datasets:
        ...
        for i, t in enumerate(dset['tasks']):
            ...
            if task.startswith('mlm'):
                dataset = build_mlm_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            elif task.startswith('mrfr'):
                dataset = build_mrfr_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            elif task.startswith('mrckl'):
                dataset = build_mrc_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            elif task.startswith('itm'):
                dataset = build_itm_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            elif task.startswith('itkm'):
                dataset = build_itkm_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            elif task.startswith('mkm'):
                dataset = build_mkm_dataset(txt_path, img_path, all_img_dbs, is_train, opts)
            else:
...
def main(opts):
    ...
    if opts.itm_ot_lambda > 0:
        for task in train_dataloaders.keys():
            if task.startswith('itm') or task.startswith('itkm'):
...
def validate(model, val_dataloaders):
    ...
    for task, loader in val_dataloaders.items():
        ...
        if task.startswith('mlm'):
            ...
            val_log = validate_itkm(model, loader)
        else:
            raise ValueError(f'Undefined task {task}')
...
@torch.no_grad()
def validate_itkm(model, val_loader):
    LOGGER.info("start running ITKM validation...")
    val_loss = 0
    tot_ot_loss = 0
    tot_ot_pos = 0
    tot_ot_neg = 0
