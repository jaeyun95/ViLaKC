...
def main(opts):
    ...
    eval_dataset = VcrEvalDataset(
        "val", eval_txt_db, img_db=eval_img_db,
...
def evaluate(model, eval_loader):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        #print("qids : ", qids, ", curr_qa_score : ",softmax(scores[:, :4]),", curr_qar_score : ",softmax(qar_scores),
        #         'correct_qa : ',qa_targets,', correct_qar : ',qar_targets)
        pre_result['qa']  = softmax(scores[:, :4]).tolist()
        pre_result['qar'] = softmax(qar_scores).tolist()
        final_result.append(pre_result)

        #new_img_path = VCR_IMAGES_DIR + img_path
        #print(new_img_path)
        #img = mpimg.imread(new_img_path)
        #plt.imshow(img)
        #all_language_list = question[0] + answer[0][0] + answer[0][1] + answer[0][2] + answer[0][3] + rationale[0][0] + \
        #                        rationale[0][1] + rationale[0][2] + rationale[0][3]
        #img = draw_box(img, img_box[0], img_object[0],collect_num(question[0], answer[0], rationale[0]), all_language_list)
        #plt.imshow(img)
        #plt.show()
