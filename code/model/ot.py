...
def optimal_transport_dist_know(txt_emb, img_emb, know_emb, txt_pad, img_pad, know_pad,
                           beta=0.5, iteration=50, k=1):
    """ [B, M, D], [B, N, D], [B, M], [B, N]"""
    txt_img_emb = torch.cat(txt_emb, img_emb)
    txt_img_pad = torch.cat(txt_pad, img_pad)
    cost = cost_matrix_cosine(txt_img_emb, know_emb)

    # mask the padded inputs
    joint_pad = txt_img_emb.unsqueeze(-1) | know_pad.unsqueeze(-2)
    cost.masked_fill_(joint_pad, 0)
    ...
    know_len = (know_pad.size(1) - know_pad.sum(dim=1, keepdim=False)
               ).to(dtype=cost.dtype)

    T = ipot(cost.detach(), txt_len + img_len, txt_img_pad, know_len, know_pad, joint_pad,
             beta, iteration, k)
    distance = trace(cost.matmul(T.detach()))
    return distance
