...
from .ot import optimal_transport_dist, optimal_transport_dist_know
...
class UniterForPretraining(UniterPreTrainedModel):
    ...
    def init_word_embedding(self, num_special_tokens):
        ...
        self.cls = BertOnlyMLMHead(
            self.config, self.uniter.embeddings.word_embeddings.weight)
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task == 'mlm':
            ...
            return self.forward_mlm(input_ids, position_ids,
                                    ...
                                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat,
                                    txt_labels, compute_loss, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
        elif task == 'mrfr':
            ...
            return self.forward_mrfr(input_ids, position_ids,
                                     ...
                                     mrfr_feat_target, compute_loss, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
        elif task == 'itm':
            ...
            return self.forward_itm(input_ids, position_ids,
                                    img_feat, img_pos_feat,
                                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat,
                                    targets, ot_inputs, compute_loss, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)

        elif task == 'itkm':
            targets = batch['targets']
            ot_inputs = batch['ot_inputs']
            return self.forward_itkm(input_ids, position_ids,
                                    img_feat, img_pos_feat,
                                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat,
                                    targets, ot_inputs, compute_loss, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
            ...
            return self.forward_mrc(input_ids, position_ids,
                                    ...
                                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat,
                                    img_masks, img_mask_tgt,
                                    mrc_label_target, task, compute_loss, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
        else:
            raise ValueError('invalid task')

    def forward_mlm(self, input_ids, position_ids, img_feat, img_pos_feat,
                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat,
                    txt_labels, compute_loss=True, txt_type_ids=None, know_type_ids=None):
        sequence_output = self.uniter(input_ids, position_ids,
                                      img_feat, img_pos_feat,
                                      attention_mask, node_feat, adj_feat, know_pos_feat, gather_index,
                                      output_all_encoded_layers=False, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
    ...
    def forward_mrfr(self, input_ids, position_ids, img_feat, img_pos_feat,
                     attention_mask, gather_index, node_feat, adj_feat, know_pos_feat, img_masks, img_mask_tgt,
                     feat_targets, compute_loss=True, txt_type_ids=None, know_type_ids=None):
        sequence_output = self.uniter(input_ids, position_ids,
                                      img_feat, img_pos_feat,
                                      attention_mask, node_feat, adj_feat, know_pos_feat, gather_index,
                                      output_all_encoded_layers=False,
                                      img_masks=img_masks, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
    ...
    def forward_itm(self, input_ids, position_ids, img_feat, img_pos_feat,
                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat, targets, ot_inputs,
                    compute_loss=True, txt_type_ids=None, know_type_ids=None):
        sequence_output = self.uniter(input_ids, position_ids,
                                      img_feat, img_pos_feat,
                                      attention_mask, node_feat, adj_feat, know_pos_feat, gather_index,
                                      output_all_encoded_layers=False, txt_type_ids=txt_type_ids, know_type_ids=know_type_ids)
        ...
        if ot_inputs is not None:
            ...
            il = img_feat.size(1)
            kl = node_feat.size(1)
            max_l = max(ot_inputs['scatter_max'] + 1, tl+il+kl)
            ...
            ctx_emb = torch.zeros(b, max_l, self.config.hidden_size,
                                  ...
                                  device=sequence_output.device
                                             ...
                                             src=sequence_output)
            #txt_emb = ctx_emb[:, :tl, :]
            txt_know_emb = ctx_emb[:, :tl+kl, :]
            img_emb = ctx_emb[:, tl+kl:tl+il+kl, :]

            txt_pad = ot_inputs['txt_pad']
            img_pad = ot_inputs['img_pad']
            know_pad = ot_inputs['know_pad']
            txt_know_pad = torch.cat((txt_pad, know_pad),1)
            # NOTE: run in fp32 for stability
            ot_dist = optimal_transport_dist(txt_know_emb.float(), img_emb.float(),txt_know_pad, img_pad).to(txt_know_emb)
    ...
    def forward_itkm(self, input_ids, position_ids, img_feat, img_pos_feat,
                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat, targets, ot_inputs,
                    compute_loss=True, txt_type_ids=None, know_type_ids=None):
        sequence_output = self.uniter(input_ids, position_ids,
                                      img_feat, img_pos_feat,
        ...
        if ot_inputs is not None:
            ...
            ctx_emb = torch.zeros(b, max_l, self.config.hidden_size,
                                  ...
                                  device=sequence_output.device
                                             ...
                                             src=sequence_output)
            txt_img_emb = ctx_emb[:, :tl+il, :]
            #img_emb = ctx_emb[:, tl:tl+il, :]
            know_emb = ctx_emb[:, tl+il:tl+il+kl]

            txt_pad = ot_inputs['txt_pad']
            img_pad = ot_inputs['img_pad']
            know_pad = ot_inputs['know_pad']
            txt_img_pad = torch.cat((txt_pad,img_pad),1)


            # NOTE: run in fp32 for stability
            ot_dist = optimal_transport_dist(txt_img_emb.float(), know_emb.float(), txt_img_pad, know_pad).to(txt_img_emb)
            #ot_dist = optimal_transport_dist_know(txt_emb.float(), img_emb.float(), know_pad.float(), txt_pad, img_pad, know_pad).to(txt_emb)
    ...
    def forward_mrc(self, input_ids, position_ids, img_feat, img_pos_feat,
                    attention_mask, gather_index, node_feat, adj_feat, know_pos_feat, img_masks, img_mask_tgt,
                    label_targets, task, compute_loss=True, txt_type_ids=None, know_type_ids=None):
        sequence_output = self.uniter(input_ids, position_ids,
                                      img_feat, img_pos_feat,
                                      attention_mask, node_feat, adj_feat, know_pos_feat, gather_index,
                                      output_all_encoded_layers=False,
